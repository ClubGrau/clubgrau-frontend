...
describe('toastMessageForRemoveError', () => {
  it('does not emit a remove toast on 401', () => {
    expect(
      toastMessageForRemoveError(httpError(401, 'Authentication failed')),
    ).toBeNull()
  })

  it('maps 403 to Ação não permitida', () => {
    expect(toastMessageForRemoveError(httpError(403, 'Action not allowed'))).toBe(
      'Ação não permitida.',
    )
  })

  it('maps Last Admin 409 to the operator copy and does not use the English API string', () => {
    const message = toastMessageForRemoveError(
      httpError(409, 'Last Admin must stay ACTIVE until another Admin exists'),
    )
    expect(message).toBe(
      'É preciso existir outro Administrador ativo antes desta ação.',
  ...
  it('maps NOT_INACTIVE and ALREADY_REMOVED 409s to the API error string', () => {
    expect(
      toastMessageForRemoveError(httpError(409, 'Employee is not inactive')),
    ).toBe('Employee is not inactive')
    expect(
      toastMessageForRemoveError(httpError(409, 'Employee is already removed')),
    ).toBe('Employee is already removed')
  })

  it('maps 400 and other 409s to the API error string', () => {
    expect(toastMessageForRemoveError(httpError(400, 'Invalid payload'))).toBe(
      'Invalid payload',
    )
    expect(toastMessageForRemoveError(httpError(409, 'Something else'))).toBe(
      'Something else',
    )
  })

  it('returns the API string for unknown failures when present', () => {
    expect(toastMessageForRemoveError(httpError(500, 'Internal'))).toBe('Internal')
  })

  it('returns null when there is no API error string', () => {
    expect(toastMessageForRemoveError(new Error('Network Error'))).toBeNull()
  })
})

describe('useEmployeeLifecycle remove', () => {
  afterEach(() => {
    dismissAllToasts()
  })
  ...
  it('calls remove with exactly { id, password } and no actorId', async () => {
    ...
    lifecycle.remove({ id: 'emp-2', password: 'secret' })
    await flushPromises()
  ...
  it('does not call the API when id or password is empty', async () => {
    ...
    lifecycle.remove({ id: '', password: 'secret' })
    lifecycle.remove({ id: 'emp-2', password: '' })
    await flushPromises()
  ...
  it('on 200 calls onRemoved and pushes the success toast', async () => {
    ...
    expect(lifecycle.removeError.value).toBeNull()
  ...
  it('on 401 sets the opaque modal error, stays without toast, and does not call callbacks', async () => {
    ...
    lifecycle.remove({ id: 'emp-2', password: 'wrong' })
    await flushPromises()

    expect(onRemoved).not.toHaveBeenCalled()
    expect(onRemoveConflict).not.toHaveBeenCalled()
    expect(toasts.value).toHaveLength(0)
    expect(lifecycle.removeError.value).toBe(
  ...
  it('on NOT_INACTIVE 409 toasts the API error and calls onRemoveConflict', async () => {
    const onRemoved = vi.fn()
    const onRemoveConflict = vi.fn()
    const api = stubApi({

...
3. On `200`, patch the snapshot status from the response and stay on the same detail, now `ACTIVE`. Symmetric to Deactivate: do not switch the list tab, do not close the drawer.
4. Triggering from the row menu with no drawer open opens the detail on that Target after success, matching Deactivate.
